...
class ProjectForm(FlaskForm):
    name = StringField("Название", validators=[DataRequired()])
    description = TextAreaField("Описание", validators=[DataRequired()])
    responsible_department_id = SelectField("Ответственный отдел", choices=[], coerce=int, validators=[DataRequired()])
...
class EmployeeForm(FlaskForm):
    ...
    hire_date = DateField("Дата приема на работу", format='%Y-%m-%d', validators=[DataRequired()])
